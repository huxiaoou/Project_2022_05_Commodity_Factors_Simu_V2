...
check_and_mkdir(evaluation_robust_dir)
check_and_mkdir(os.path.join(evaluation_robust_dir, method))
check_and_mkdir(os.path.join(evaluation_robust_dir, method, "by_comb_id"))

aver_nav_summary_data = []
for hold_period_n, single_hold_prop in product(robust_hold_period_n_list, robust_single_hold_prop_list):
    ...
    nav_path = os.path.join(evaluation_robust_dir, method, "by_comb_id", nav_file)
    ...
    plot_lines(
        ...
        t_save_dir=os.path.join(evaluation_robust_dir, method, "by_comb_id")
    )
...
aver_nav_summary_path = os.path.join(evaluation_robust_dir, method, aver_nav_summary_file)
...
for evaluation_idx in ["年化收益", "夏普比率"]:
    ...
    sorted_summary_path = os.path.join(evaluation_robust_dir, method, sorted_summary_file)
    ...
    plot_lines(
        ...
        t_save_dir=os.path.join(evaluation_robust_dir, method))
